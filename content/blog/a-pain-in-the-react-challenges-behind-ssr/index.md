...
As I've said at the start of the post most of the problems appear because React doesn't provide us with a standard data fetching solution. We have to implement one of our own on top of the react api. All of this might change with the introduction of [Suspense](https://reactjs.org/docs/code-splitting.html#reactlazy), Concurrent and [Cache](https://www.npmjs.com/package/react-cache). The issue is that _is not ready yet_. React doesn't support suspense on server side rendering. And on the client it's only supported to do code splitting via `React.lazy`. [React cache is on its early alpha](https://github.com/facebook/react/issues/14780) and many things are prefixed with an `unstable`. Although there are many experiments out there (such as [react ssr prepass](https://github.com/FormidableLabs/react-ssr-prepass) or [react lightyear](https://github.com/Ephem/react-lightyear)), we won't be able to use them for anything more than playing around with what might be the future. But, let's cross our fingers that [React's data fetching future might look bright](https://reactjs.org/blog/2018/11/27/react-16-roadmap.html), we just need to `await React.suspense()`.